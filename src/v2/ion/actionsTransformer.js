...
/**
 *
 * @param {AuthnResponse} resp transformed authn v2 response
...
const transformActions = transformedResp => {
  ...
  const transformedCurrentState = {};
  _.each(transformedResp.currentState, (val, key) => {
    if (_.isFunction(val)) {
      transformedCurrentState[key] = convertActionToRequestFunction(val);
    } else {
      transformedCurrentState[key] = val;
    }
  });
  transformedResp.currentState = transformedCurrentState;
